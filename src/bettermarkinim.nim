...
var
  ...
  betterMarkovs {.threadvar.}: Table[int64, (int64, MarkovGenerator)] # (chatId): (timestamp, BetterMarkovChain)
...
const
  ...
  BETTERMARKOV_DB = "bettermarkov.db"

  ANTIFLOOD_SECONDS = 10
  ANTIFLOOD_RATE = 6

  BETTERMARKOV_SAMPLES_CACHE_TIMEOUT = 60 * 30 # 30 минут
  GROUP_ADMINS_CACHE_TIMEOUT = 60 * 5 # результат действителен в течение пяти минут
  BETTERMARKOV_CHAT_SESSIONS_TIMEOUT = 60 * 30 # 30 минут
...
proc refillBetterMarkov(conn: DbConn, session: Session) =
  for message in conn.getLatestMessages(session = session, count = keepLast):
    if session.isMessageOk(message.text):
      betterMarkovs.get(session.chat.chatId).addSample(message.text, asLower = not session.caseSensitive)
...
proc cleanerWorker {.async.} =
  while true:
    ...
    let betterMarkovsKeys = betterMarkovs.keys.toSeq()
    for record in betterMarkovsKeys:
      let (timestamp, _) = betterMarkovs[record]
      if time - timestamp > BETTERMARKOV_SAMPLES_CACHE_TIMEOUT:
        betterMarkovs.del(record)

    let chatSessionsKeys = chatSessions.keys.toSeq()
    for record in chatSessionsKeys:
      let (timestamp, _) = chatSessions[record]
      if time - timestamp > BETTERMARKOV_CHAT_SESSIONS_TIMEOUT:
...
proc getSettingsKeyboard(session: Session): InlineKeyboardMarkup =
  ...
  return newInlineKeyboardMarkup(
    ...
    @[
      InlineKeyboardButton(text: &"Отключить /bettermarkov {asEmoji(session.chat.markovDisabled)}", callbackData: some &"bettermarkov_{chatId}"),
...
proc handleCommand(bot: Telebot, update: Update, command: string, args: seq[string], dbUser: database.User) {.async, gcsafe.} =
  ...
  of "deleteme":
    ...
    if len(args) > 0 and args[0] == "confirm":
      ...
      discard await bot.sendMessage(message.chat.id,
        ...
        "\nПримечание: некоторые сообщения могут по-прежнему кэшироваться в памяти бота в скомпилированной модели bettermarkov, они скоро истекут (максимум через 4 часа, после того как бот перезапустится для процедуры резервного копирования)" &
  ...
  of "help":
    if message.chat.kind.endswith("group") and not isSenderAdmin:
      return
    discard await bot.sendMessage(message.chat.id, HELP_TEXT, messageThreadId=threadId)
  of "privacy":
  ...
  of "count", "stats":
    ...
    var statsMessage = &"*Пользователи*: {conn.getCount(database.User)}\n" &
      ...
      &"*Кэшированные bettermarkovs*: {len(betterMarkovs)}\n" &
      &"*Время работы*: {toInt(epochTime() - uptime)}s\n" &
      &"*Размер базы данных*: {humanBytes(getFileSize(DATA_FOLDER / BETTERMARKOV_DB))}\n" &
  ...
  of "bettermarkov", "quote":
    ...
    if not betterMarkovs.hasKey(message.chat.id):
      betterMarkovs[message.chat.id] = (unixTime(), newMarkov(@[]))
      conn.refillBetterMarkov(cachedSession)

    if len(betterMarkovs.get(message.chat.id).samples) == 0:
    ...
    {.cast(gcsafe).}:
      let generator = betterMarkovs.get(message.chat.id)
      let generated = try:
  ...
  of "wouldyourather":
    ...
    if len(betterMarkovs.get(message.chat.id).samples) < 10:
  ...
  #[ of "export":
    if senderId notin admins:
      ...
      return
    let tmp = getTempDir()
    copyFileToDir(DATA_FOLDER / BETTERMARKOV_DB, tmp)
    discard await bot.sendDocument(senderId, "file://" & (tmp / BETTERMARKOV_DB))
    discard tryRemoveFile(tmp / BETTERMARKOV_DB) ]#
  of "settings":
    if message.chat.kind.endswith("group") and not isSenderAdmin:
      discard await bot.sendMessage(message.chat.id, UNALLOWED, messageThreadId=threadId)
      return
  ...
  of "delete":
    ...
    if message.chat.kind.endswith("group") and not isSenderAdmin:
      ...
      try:
        ...
        if betterMarkovs.hasKey(message.chat.id):
          betterMarkovs.del(message.chat.id)
  ...
  try:
    block callbackBlock:
      ...
      of "set":
        ...
        betterMarkovs[chatId] = (unixTime(), newMarkov(
...
proc updateHandler(bot: Telebot, update: Update): Future[bool] {.async, gcsafe.} =
  ...
  try:
    ...
    if response.text.isSome or response.caption.isSome:
      ...
      if not betterMarkovs.hasKeyOrPut(chatId, (unixTime(), newMarkov(@[]))):
        conn.refillBetterMarkov(cachedSession)
      else:
        betterMarkovs.get(chatId).addSample(text, asLower = not cachedSession.caseSensitive)
      ...
      if (rand(1 .. 100) <= percentage or (percentage > 0 and repliedToMarkinim and cachedSession.alwaysReply)) and not isFlood(chatId, rate = 10, seconds = 30):
        ...
        {.cast(gcsafe).}:
          let generated = betterMarkovs.get(chatId).generate()
        if generated.isSome:
          var text = generated.get()
          if cachedSession.owoify != 0:
            {.cast(gcsafe).}:
...
proc main {.async.} =
  ...
  conn = initDatabase(BETTERMARKOV_DB)
  defer: conn.close()
